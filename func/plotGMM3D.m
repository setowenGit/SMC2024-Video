...
darkcolor = max(col1-0.1,0);
versecolor = abs(col1-1);

nbData = size(Mu,2);
nbPoints = 20; % 20 nb of points to form a circular path
nbRings = 15; % 10 Number of circular paths following the principal direction
	...
	if dispOpt==100
		...
		for j=1:size(xringfull,3)-1
			for i=1:size(xringfull,2)-1
				...
				if dispOpt==1
                    h = patch(xTmp(1,:),xTmp(2,:),xTmp(3,:), col1,'edgecolor',max(col1-0.1,0),'linewidth',1);
				elseif dispOpt==2
                    h = patch(xTmp(1,:),xTmp(2,:),xTmp(3,:), col1,'edgecolor',max(col1-0.1,0),'linewidth',1,'facealpha',alpha,'edgealpha',alpha);
                elseif dispOpt==3
					h = patch(xTmp(1,:),xTmp(2,:),xTmp(3,:), min(col1,1),'linestyle','none','facealpha',alpha);
                elseif dispOpt==4
                    h = patch(xTmp(1,:),xTmp(2,:),xTmp(3,:), col1);
				end
			end
		end
    end
    if dispOpt~=3
        plot3(Mu(1,:), Mu(2,:), Mu(3,:), '.', 'markersize', 15, 'color', versecolor)
    end
end